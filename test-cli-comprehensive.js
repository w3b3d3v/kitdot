...
const TEST_DIR = "/tmp/kitdot-cli-tests";
...
async function runCLITest(projectName, projectType) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (data) => {
      ...
      if (
        data.toString().includes("Choose a frontend template") &&
        !processExited
      ) {
        setTimeout(() => {
          if (child.stdin.writable && !processExited) {
...
async function runAllTests() {
  ...
  for (const testCase of testCases) {
    try {
      ...
      if (result.success) {
        console.log(
          `✅ ${testCase.name} (${testCase.type}): CLI execution successful`
        );
