...
import { getTemplatesForMode, getAllTemplatesByCategory, filterTemplatesByTypes } from '../src/templates/registry.js';
...
async function testModeBasedTemplateFiltering() {
  ...
  assert(fullstackModeTemplates.length >= 3, 'Fullstack mode should show all templates');

  // Should include templates from all categories
  const hasFrontend = fullstackModeTemplates.some(t => t.category === 'frontend');
  const hasSmartcontract = fullstackModeTemplates.some(t => t.category === 'smartcontract');
  const hasFullstack = fullstackModeTemplates.some(t => t.category === 'fullstack');

  assert(hasFrontend, 'Fullstack mode should include frontend templates');
  assert(hasSmartcontract, 'Fullstack mode should include smartcontract templates');
  ...
  const smartcontractsOnlyTemplates = getTemplatesForMode('smartcontracts-only');
  for (const template of smartcontractsOnlyTemplates) {
    assert.equal(template.category, 'smartcontract',
...
async function testTemplateCategorization() {
  ...
  assert(allTemplatesByCategory.smartcontract, 'Smartcontract category should exist');
  ...
  const hasDefaultFrontend = frontendTemplates.some(t => t.key === 'default-frontend');
  const hasDefaultContracts = smartcontractTemplates.some(t => t.key === 'default-contracts');
  const hasDefaultFullstack = fullstackTemplates.some(t => t.key === 'default');

  assert(hasDefaultFrontend, 'Frontend category should include default-frontend');
  assert(hasDefaultContracts, 'Smartcontract category should include default-contracts');
...
async function testMultiCategoryFiltering() {
  ...
  const frontendAndSmartcontract = filterTemplatesByTypes(['frontend', 'smartcontract']);

  // Should only contain frontend and smartcontract templates
  for (const template of frontendAndSmartcontract) {
    assert(['frontend', 'smartcontract'].includes(template.category),
  ...
  const hasFrontend = frontendAndSmartcontract.some(t => t.category === 'frontend');
  const hasSmartcontract = frontendAndSmartcontract.some(t => t.category === 'smartcontract');
  const hasFullstack = frontendAndSmartcontract.some(t => t.category === 'fullstack');

  assert(hasFrontend, 'Multi-category filter should include frontend templates');
  assert(hasSmartcontract, 'Multi-category filter should include smartcontract templates');
  assert(!hasFullstack, 'Multi-category filter should NOT include fullstack templates when not selected');
...
async function testSDKPathResolution() {
  ...
  try {
    const { findSDKRoot, resolveTemplatePath, templateExists } = await import('../src/utils/sdk-paths.js');
    ...
    assert(typeof templatePath === 'string', 'Template path should be a string');
    assert(templatePath.includes('templates/default'), 'Template path should contain the relative path');

    // Test template existence check
    const exists = await templateExists('templates/default');
    assert(typeof exists === 'boolean', 'Template existence check should return boolean');
...
async function testAutoPairingLogic() {
  ...
  const testCases = [
    ...
    },
    {
      ...
      description: 'Smartcontract template should auto-pair with default-frontend'
    },
    {
      mode: 'fullstack',
  ...
  for (const testCase of testCases) {
    ...
    if (testCase.expectedPairing === null) {
      ...
      assert(shouldPair, testCase.description);
      assert.equal(pairingTemplate, testCase.expectedPairing, testCase.description);
    }
  }
