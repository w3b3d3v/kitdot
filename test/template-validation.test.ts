...
const TEST_TEMP_DIR = '/tmp/kit-dot-template-validation';
...
describe('Template Validation', () => {
  ...
  test('should not have templates with master branch', () => {
    ...
    const templatesWithMaster = Object.entries(TEMPLATE_REGISTRY)
      .filter(([_, template]) => 
        template.source.type === 'remote' && 
        template.source.branch === 'master'
      );
    ...
    if (templatesWithMaster.length > 0) {
      ...
      throw new Error(`Templates using 'master' branch found: ${templateNames}. Update to 'main' or correct branch.`);
    }
  });
  ...
  test('should validate templates with optionalSetup instructions', () => {
    ...
    if (templatesWithOptionalSetup.length > 0) {
      console.log('Templates with optional setup instructions:', 
