...
async function testFrontendOnlyFiltering() {
  ...
  const fullstackTemplateKeys = Object.keys(TEMPLATE_REGISTRY).filter(
    (key) => TEMPLATE_REGISTRY[key].category === "fullstack"
  );

  for (const template of frontendTemplates) {
    assert(
      !fullstackTemplateKeys.includes(template.key),
...
async function testFullstackFiltering() {
  ...
  const frontendTemplateKeys = Object.keys(TEMPLATE_REGISTRY).filter(
    (key) => TEMPLATE_REGISTRY[key].category === "frontend"
  );

  for (const template of fullstackTemplates) {
    assert(
      !frontendTemplateKeys.includes(template.key),
...
async function testNoDuplicateDefaults() {
  ...
  const frontendDefaults = frontendTemplates.filter((t) =>
    t.key.includes("default")
  );
  const fullstackDefaults = fullstackTemplates.filter((t) =>
    t.key.includes("default")
  );
  const backendDefaults = backendTemplates.filter((t) =>
    t.key.includes("default")
  );

  // Each category should have exactly one default template
  assert.equal(
    frontendDefaults.length,
    1,
    "Frontend should have exactly one default template"
  );
  assert.equal(
    fullstackDefaults.length,
    1,
    "Fullstack should have exactly one default template"
  );
  assert.equal(
    backendDefaults.length,
    1,
    "Smartcontract should have exactly one default template"
  );

  // Verify the correct default template for each category
  assert.equal(
    frontendDefaults[0].key,
    "default-frontend",
    "Frontend default should be default-frontend"
  );
  assert.equal(
    fullstackDefaults[0].key,
    "default",
    "Fullstack default should be default"
  );
  assert.equal(
    backendDefaults[0].key,
    "default-contracts",
    "Smartcontract default should be default-contracts"
  );
...
async function testDefaultIndicationLogic() {
  ...
  for (const { type, expectedDefault } of projectTypes) {
    const templates = getTemplatesByCategory(
      type as "frontend" | "fullstack" | "smartcontract"
    );
    ...
    for (const template of templates) {
      ...
      if (template.key === expectedDefault) {
        assert(
          isDefault,
          `Template ${template.key} should be marked as default for ${type} projects`
        );
      } else {
        assert(
          !isDefault,
          `Template ${template.key} should NOT be marked as default for ${type} projects`
        );
      }
    }
  }
...
async function testNoCrossCategoryContamination() {
  ...
  for (const category of allCategories) {
    const templates = getTemplatesByCategory(
      category as "frontend" | "fullstack" | "smartcontract"
    );
  ...
  const frontendFullstackOverlap = frontendKeys.filter((key) =>
    fullstackKeys.includes(key)
  );
  const frontendBackendOverlap = frontendKeys.filter((key) =>
    backendKeys.includes(key)
  );
  const fullstackBackendOverlap = fullstackKeys.filter((key) =>
    backendKeys.includes(key)
  );

  assert.equal(
    frontendFullstackOverlap.length,
    0,
    "Frontend and fullstack categories should not overlap"
  );
  assert.equal(
    frontendBackendOverlap.length,
    0,
    "Frontend and backend categories should not overlap"
  );
  assert.equal(
    fullstackBackendOverlap.length,
    0,
    "Fullstack and backend categories should not overlap"
  );
...
async function testTemplateSelectionFlow() {
  ...
  for (const projectType of projectTypes) {
    ...
    assert(
      ...
      `Should have templates available for ${projectType}`
    );
    ...
    const hasDefault = availableTemplates.some((template) => {
      if (projectType === "frontend")
        return template.key === "default-frontend";
      if (projectType === "backend")
        return template.key === "default-contracts";
    ...
    assert(
      hasDefault,
      `Should have a default template for ${projectType} projects`
    );
  }
