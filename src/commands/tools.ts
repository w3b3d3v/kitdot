...
async function displayToolsHelp() {
  console.log(chalk.blue("\n🔧 kitdot Tools Management\n"));
  console.log("Available commands:");
  console.log(
    chalk.green("  kitdot tools install-rust") +
      "  - Install Rust toolchain for blockchain development"
  );
  console.log(
    chalk.green("  kitdot tools check") +
      "        - Check status of development tools"
  );
  console.log(
    chalk.green("  kitdot tools") + "             - Show this help message"
  );
  console.log("\nFor more information: https://kitdot.dev/docs/tools\n");
}
...
async function installRustCommand() {
  ...
  console.log(
    chalk.blue(
      `🖥️  Platform detected: ${platformInfo.platform} (${platformInfo.architecture})`
    )
  );
  ...
  if (!platformInfo.isSupported) {
    ...
    console.log(
      chalk.blue(
        platformDetector.getInstallationGuidance(platformInfo.platform)
      )
    );
    return;
  }
  ...
  const timeEstimate = rustInstaller.getInstallationTimeEstimate(
  ...
  if (!rustAnswer.installRust) {
    ...
    console.log(
      chalk.blue(
        "💡 You can install Rust manually by visiting: https://rustup.rs/"
      )
    );
    return;
  }
  ...
  const spinner = ora(
    "Installing Rust toolchain for blockchain development..."
  ).start();

  try {
    const installResult = await rustInstaller.installRust(
      platformInfo.platform
    );
    ...
    if (installResult.success) {
      if (installResult.skipped) {
        ...
        spinner.succeed(
          `Rust toolchain installed successfully: ${installResult.version}`
        );
      }
    } else {
      ...
      console.log(
        chalk.blue(
          rustInstaller.getTroubleshootingGuidance(
    ...
    spinner.fail("Rust installation failed");
    console.log(
      chalk.red("❌ Unexpected error during Rust installation:", error)
    );
  }
}
...
async function checkToolsCommand() {
  ...
  console.log(
    chalk.blue(
      `📦 Platform supported: ${platformInfo.isSupported ? "Yes" : "No"}`
    )
  );
  ...
  if (isRustInstalled) {
    ...
    console.log(chalk.gray("   Run: kitdot tools install-rust"));
  }
