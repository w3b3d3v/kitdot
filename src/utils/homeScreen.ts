...
/**
 * Creates kitdot ASCII art in the style of the reference BMAD file
...
export function displayHomeScreen(): void {
  ...
  console.log(
    decorations.sideBorder + decorations.spacer + decorations.sideBorder
  );
  ...
  textLines.forEach((line) => {
    ...
    const paddedLine =
      " ".repeat(padding) + line + " ".repeat(54 - cleanLine.length - padding);
  ...
  const taglinePadding = Math.max(
    0,
    Math.floor((54 - cleanTagline.length) / 2)
  );
  const paddedTagline =
    " ".repeat(taglinePadding) +
    tagline +
    " ".repeat(54 - cleanTagline.length - taglinePadding);
  ...
  console.log(
    chalk.blue.bold("🚀 Welcome to the Polkadot development ecosystem!")
  );
  console.log(
    chalk.gray("This tool will help you build amazing Dapps on Polkadot Cloud")
  );
  console.log("");
}
...
export function displayTextStyleOptions(): void {
  console.log(chalk.yellow("\nStyle 1 - kitdot Large (Reference Style):"));
